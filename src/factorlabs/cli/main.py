...
def run(
    ...
    Downloads data, runs backtest with equal weights, shows metrics and charts.

    Example:
        factorlab run AAPL MSFT GOOGL
        factorlab run AAPL MSFT --start 2024-01-01 --end 2024-06-01 --cash 50000
        factorlab run AAPL MSFT GOOGL --save-charts
        factorlab run AAPL MSFT --no-charts
    """
    typer.echo("\n" + "=" * 50)
    ...
    # Step 2: Calculate equal weights (leaving 3% buffer for costs)
    typer.echo(f"\n⚖️  Step 2: Setting up equal-weight strategy...")
    n_tickers = len(tickers)
    weight_per_ticker = 0.97 / n_tickers  # 97% invested, 3% buffer for costs
    weights = {t.lower(): weight_per_ticker for t in tickers}
    ...
    result = backtester.run(prices=prices, strategy=strategy, config=bt_config)
    ...
    if not no_charts:
        ...
        # Equity curve chart
        pltxt.clear_figure()
        pltxt.plot(values, marker="braille")
        pltxt.title("Portfolio Equity Curve")
        pltxt.xlabel("Time")
        pltxt.ylabel("Value ($)")
